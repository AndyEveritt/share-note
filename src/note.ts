...
export default class Note {
  ...
  async share(file: TFile | null, depth: number = 0) {
    ...
    this.status = new StatusMessage(
      "Parsing note content, please do not change to another note while this message is displayed.",
      StatusType.Default,
      60 * 1000
    );
    ...
    if (this.plugin.settings.removeYaml) {
      ...
      this.contentDom
        .querySelectorAll("input.metadata-property-key-input")
        .forEach((el) => {
    ...
    // Replace links
    for (const el of this.contentDom.querySelectorAll<HTMLElement>(
      "a.internal-link"
    )) {
      ...
      if (href?.match(/^#/)) {
        ...
        if (this.contentDom.querySelectorAll(selector)?.[0]) {
          el.setAttribute(
            "onclick",
            `document.querySelectorAll('${selector}')[0].scrollIntoView(true)`
          );
        }
    ...
    if (this.meta?.frontmatter?.[this.field(YamlField.link)]) {
      const match = parseExistingShareUrl(
        this.meta.frontmatter[this.field(YamlField.link)]
      );
      if (match) {
          ...
          if (count > parsing) {
            ...
            if (sections.length > 12) {
              sections
                ...
                .forEach((section: PreviewSection) => {
  ...
  getCalloutIcon(test: (selectorText: string) => boolean) {
    const rule = this.cssRules.find(
      (rule: CSSStyleRule) =>
        ...
        rule.style.getPropertyValue("--callout-icon")
    ) as CSSStyleRule;
    if (rule) {
  ...
  getExpiration() {
    ...
    const expiration =
      this.getProperty(YamlField.expires) || this.plugin.settings.expiry;
    if (expiration) {
