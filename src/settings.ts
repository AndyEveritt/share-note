...
export class ShareSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            this.plugin.settings.yamlField =
              value || DEFAULT_SETTINGS.yamlField;
            await this.plugin.saveSettings();
          })
      );
    ...
    new Setting(containerEl)
      .setName(
        `⭐ Your shared note theme is "${
          this.plugin.settings.theme || "Obsidian default theme"
        }"`
      )
      .setDesc(
        ...
        dropdown
          ...
          .onChange(async (value) => {
            this.plugin.settings.themeMode =
              ThemeMode[value as keyof typeof ThemeMode];
            await this.plugin.saveSettings();
          });
      });
    ...
    const titleSetting = new Setting(containerEl)
      ...
      .addDropdown((dropdown) => {
        ...
        dropdown
          ...
          .onChange(async (value) => {
            this.plugin.settings.titleSource =
              TitleSource[value as keyof typeof TitleSource];
            if (
              this.plugin.settings.titleSource ===
              TitleSource["Frontmatter property"]
            ) {
              titleSetting.setDesc(
                "Set the title you want to use in a frontmatter property called `" +
                  this.plugin.field(YamlField.title) +
                  "`"
              );
            } else {
