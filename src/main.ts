...
export default class SharePlugin extends Plugin {
  ...
  async copyShareLink(file: TFile): Promise<string | undefined> {
    ...
    const shareLink =
      meta?.frontmatter?.[
        this.settings.yamlField + "_" + YamlField[YamlField.link]
      ];
    if (shareLink) {
    ...
    const timer = setInterval(() => {
      ...
      const shareLink =
        this.app.metadataCache.getFileCache(activeFile)?.frontmatter?.[
          this.field(YamlField.link)
        ];
      if (!shareLink) return;
      document
        .querySelectorAll(
          `div.metadata-property[data-property-key="${this.field(
            YamlField.link
          )}"]`
        )
        .forEach((propertyEl) => {
          ...
          const linkEl = valueEl?.querySelector(
            "div.external-link"
          ) as HTMLElement;
